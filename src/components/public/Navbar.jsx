      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-12 sm:h-14">
          ...
          <Link to="/" className="flex-shrink-0 flex items-center gap-2 sm:gap-3 group">
            <div className={`w-10 h-10 sm:w-10 sm:h-10 rounded-xl flex items-center justify-center text-black font-bold text-lg sm:text-xl transition-all duration-300 transform group-hover:rotate-6 ${scrolled ? 'bg-brand-600 scale-90' : 'bg-white scale-100'
              }`}>
            ...
            <span className={`font-heading font-black text-base sm:text-xl tracking-tighter transition-all duration-300 ${scrolled ? 'text-gray-900' : 'text-white drop-shadow-md'
              }`}>
          ...
          <div className="hidden lg:flex space-x-1 items-center">
            {navLinks.map((link) => (
              <Link
                key={link.name}
                to={link.path}
                className={`px-4 py-2 rounded-xl text-sm font-bold transition-all duration-300 ${isActive(link.path)
            ...
            <div className="ml-4 pl-4 border-l border-white/20">
              <Link
                to="/quote"
                className={`px-6 py-2 rounded-xl text-xs font-black uppercase tracking-widest shadow-xl transition-all duration-300 transform hover:-translate-y-1 active:scale-95 ${scrolled
          ...
          <div className="lg:hidden flex items-center">
            <button
              ...
              className={`p-3 sm:p-3 rounded-xl transition-colors ${scrolled ? 'text-gray-900 hover:bg-gray-100' : 'text-white hover:bg-white/20'
                }`}
              ...
              {navLinks.map((link) => (
                <motion.div key={link.name} variants={{
                  ...
                  <Link
                    ...
                    className={`block px-4 sm:px-5 py-3 sm:py-4 rounded-2xl text-base sm:text-lg font-bold transition-all ${isActive(link.path)
